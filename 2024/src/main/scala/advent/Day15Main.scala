...
//  println(s"Robot position: $getRobotPos")
  ...
  private def getObjectAtPost(pos: (Int, Int)): Option[Object] = {
    map.find(o => o.pos._2 == pos._2 && o.pos._1 <= pos._1 && pos._1 < o.pos._1 + o.width)
  }
  ...
  private def move(pos: (Int, Int), direction: Direction): Boolean = {
    val obj = getObjectAtPost(pos).get
    ...
    val newPositions: Seq[(Int, Int)] = Math.abs(direction.dy) match {
      ...
      case _ => {
        if(direction.dx == 1) {
          val o = getObjectAtPost(newPos)
          if(o.isDefined && o.get == obj)
            Seq((newPos._1 + 1, newPos._2))
          else Seq(newPos)
        }
        else Seq(newPos)
      }
    ...
    if(newPositions.forall(isFree)) { // If all new positions are free
      ...
      val boxes = newPositions.filter(isBox).map(p => getObjectAtPost(p).get).toSet
  ...
  private def printMap(): Unit = {
    val maxX = map.map(_.pos._1).max + map.map(_.width).max - 1
